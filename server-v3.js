...
const lipsync = require('./lib/lipsync-client');
const {
...
app.get('/', (req, res) => res.json({
  ok: true,
  app: 'ViralVoice API',
  version: '3.0.0-musetalk-lipsync'
}));

app.get('/api/health', async (req, res) => {
  const lipSyncHealth = await lipsync.getHealth();
  res.json({
    ok: true,
    app: 'ViralVoice API',
    version: '3.0.0-musetalk-lipsync',
    ...
    lipSyncConfigured: lipSyncHealth.configured,
    lipSyncReady: lipSyncHealth.ready,
    lipSyncEngine: lipSyncHealth.engine,
    lipSyncGpu: lipSyncHealth.gpu,
    lipSyncDetail: lipSyncHealth.detail,
    lipSyncMaxDurationSeconds: config.LIPSYNC_MAX_DURATION_SECONDS
  });
});
...
app.post('/api/dub-video', upload.single('media'), async (req, res) => {
  ...
  try {
    ...
    const options = {
      ...
      lipSyncRequested: String(req.body.lipSync || 'false') === 'true',
      lipSyncQuality: cleanLipSyncQuality(req.body.lipSyncQuality),
      lipSyncBboxShift: clampInteger(req.body.lipSyncBboxShift, -20, 20),
      lipSyncExtraMargin: clampInteger(req.body.lipSyncExtraMargin, 0, 40)
    };
    ...
    const lipSyncVideoPath = path.join(config.OUTPUT_DIR, `${jobId}-viralvoice-lipsync.mp4`);
    ...
    const payload = {
      ...
      lipSyncRequested: options.lipSyncRequested,
      lipSyncUsed: false,
      lipSyncEngine: null,
      lipSyncWarning: null,
    ...
    if (isVideo) {
      ...
      if (options.lipSyncRequested) {
        if (duration > config.LIPSYNC_MAX_DURATION_SECONDS) {
          payload.lipSyncWarning =
            `Vidéo trop longue pour le mode visage : ${duration.toFixed(1)} s. ` +
            `Limite GPU : ${config.LIPSYNC_MAX_DURATION_SECONDS} s.`;
        } else if (!lipsync.isConfigured()) {
          payload.lipSyncWarning = 'Moteur MuseTalk non configuré sur Render.';
        } else {
          try {
            console.log(`[${jobId}] LIPSYNC START quality=${options.lipSyncQuality}`);
            await lipsync.generateLipSync({
              videoPath: inputPath,
            ...
            payload.dubbedVideoUrl = `/outputs/${path.basename(lipSyncVideoPath)}`;
            console.log(`[${jobId}] LIPSYNC DONE`);
          } catch (error) {
            payload.lipSyncWarning = error.message || 'Lip-sync indisponible.';
            if (config.LIPSYNC_REQUIRED) throw error;
            console.error(`[${jobId}] LIPSYNC FALLBACK`, error);
          }
        }
      }
    } else if (options.lipSyncRequested) {
      payload.lipSyncWarning = 'Le lip-sync nécessite une vidéo, pas seulement un fichier audio.';
    }

    await db.recordGeneration({
      clientId: chargedClient.id,
      prompt: payload.lipSyncUsed
        ? `Doublage lip-sync MuseTalk vers ${options.targetLanguage}`
        : `Doublage synchronisé vers ${options.targetLanguage}`,
...
function cleanLipSyncQuality(value) {
...
app.listen(config.PORT, () => {
  console.log(`ViralVoice 3.0 démarré sur ${config.PORT}`);
});
