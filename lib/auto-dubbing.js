...
function chooseRoute({ speakers, segments, targetLanguage }) {
  ...
  if (
    ...
    return {
      ...
      label: 'Traduction vocale automatique rapide',
      reason: 'Une seule voix détectée et langue compatible avec Realtime Translate.'
    };
  }

  if (speakers.length > 1) {
    return {
      id: 'segmented-multispeaker',
      label: 'Doublage multi-intervenants synchronisé',
      reason: `${speakers.length} intervenants détectés : moteur segmenté multi-voix.`
    };
  }

  if (!realtimeSupported) {
    return {
      id: 'segmented-language-fallback',
      label: 'Doublage synchronisé compatible langue',
      reason: 'Langue cible non disponible sur Realtime Translate.'
    };
  }

  return {
    id: 'segmented-complex-single-speaker',
    label: 'Doublage synchronisé renforcé',
    reason: 'Audio très segmenté : moteur synchronisé plus stable.'
  };
}

async function runRealtimeSingleSpeaker({
  sourceAudioPath,
  segments,
  targetLanguage,
  duration,
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    console.log(
      ...
      `${segment.start.toFixed(2)}-${segment.end.toFixed(2)} ${result.engine}`
    );
  }
  ...
  return {
    translated,
    prepared,
    voiceMap: { [segments[0]?.speaker || 'A']: 'dynamic-source-adaptation' },
    voiceEngines: [config.REALTIME_TRANSLATE_MODEL],
    voiceFallbackSegments: 0,
    syncMode: 'realtime-translate-segment-timeline',
