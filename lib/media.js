...
function runTimelineMix({
  ...
  return new Promise((resolve, reject) => {
    ...
    // Le FFmpeg embarqué normalise amix en divisant le signal par le nombre
    // d'entrées. On compense immédiatement ce gain, puis on limite les rares
    // chevauchements. Cette méthode reste compatible avec l'ancien FFmpeg.
    filters.push(
      `${labels.join('')}amix=inputs=${labels.length}:duration=longest:` +
      `dropout_transition=0,volume=${mixGain},alimiter=limit=0.95,` +
      `${outputFilters}[mix]`
    );
