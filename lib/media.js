...
function runTimelineMix({
  ...
  return new Promise((resolve, reject) => {
    ...
    // IMPORTANT: amix normalise par defaut selon le nombre d'entrees.
    // Avec 20-40 segments cela attenuait la voix de plus de 20 dB.
    // normalize=0 conserve le niveau original ; le limiteur protege les chevauchements.
    filters.push(
...
async function normalizeVoice(inputPath, outputPath, voiceVolume) {
  await runVoiceNormalize(inputPath, outputPath, voiceVolume);

  let stats = null;
  try {
    stats = await detectVolumeStats(outputPath);
    console.log(
      `[AUDIO LEVEL] normalized mean=${formatDb(stats.meanVolume)} ` +
      `max=${formatDb(stats.maxVolume)}`
    );
  } catch (error) {
    console.warn('[AUDIO LEVEL] analyse impossible', error.message || error);
    return null;
  }

  // Garde-fou : une voix anormalement faible ne doit jamais etre livree au client.
  if (Number.isFinite(stats.maxVolume) && stats.maxVolume < -10) {
    const boostDb = clamp(-6 - stats.maxVolume, 0, 24);
    const boostedPath = `${outputPath}.boost.mp3`;

    try {
      await boostVoice(outputPath, boostedPath, boostDb);
      fs.renameSync(boostedPath, outputPath);
      const boostedStats = await detectVolumeStats(outputPath);
      console.log(
        `[AUDIO LEVEL] safety boost=${boostDb.toFixed(1)}dB ` +
        `mean=${formatDb(boostedStats.meanVolume)} ` +
        `max=${formatDb(boostedStats.maxVolume)}`
      );
      stats = boostedStats;
    } finally {
      try {
        if (fs.existsSync(boostedPath)) fs.unlinkSync(boostedPath);
      } catch {
...
function runVoiceNormalize(inputPath, outputPath, voiceVolume) {
...
function boostVoice(inputPath, outputPath, boostDb) {
  const safeBoost = clamp(Number(boostDb || 0), 0, 24);

  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      .audioFilters([
        `volume=${safeBoost.toFixed(2)}dB`,
        'alimiter=limit=0.95'
      ])
      .audioCodec('libmp3lame')
      .audioBitrate('160k')
...
function detectVolumeStats(inputPath) {
  return new Promise((resolve, reject) => {
    let log = '';
    ...
    ffmpeg(inputPath)
      ...
      .on('end', () => {
        const meanMatch = log.match(/mean_volume:\s*(-?[\d.]+)\s*dB/i);
        const maxMatch = log.match(/max_volume:\s*(-?[\d.]+)\s*dB/i);
        resolve({
          meanVolume: meanMatch ? Number(meanMatch[1]) : NaN,
          maxVolume: maxMatch ? Number(maxMatch[1]) : NaN
        });
      })
      .on('error', reject)
      .run();
