...
async function recoverLatestForAdmin(req) {
  ...
  if (!Number.isFinite(createdAtMs) || Date.now() - createdAtMs > maxAgeMs) return null;

  const startedAt = Number(req.query.startedAt || 0);
  if (Number.isFinite(startedAt) && startedAt > 0 && createdAtMs < startedAt - 5000) {
    return null;
  }
  ...
  if (row.result_payload?.ok) {
    return {
      ...
      recoveredGenerationCreatedAt: row.created_at
    };
  }
