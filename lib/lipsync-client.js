...
function hasServiceUrl() {
  return Boolean(config.LIPSYNC_SERVICE_URL);
}

function isConfigured() {
  // En mode obligatoire, le serveur doit tenter le moteur et échouer clairement
  // au lieu de rendre silencieusement une vidéo sans modification des lèvres.
  return hasServiceUrl() || config.LIPSYNC_REQUIRED;
}
...
async function getHealth() {
  if (!hasServiceUrl()) {
    return {
      ...
      detail: 'LIPSYNC_SERVICE_URL manquante sur Render.'
    };
  }
...
async function generateLipSync({
  ...
  if (!hasServiceUrl()) {
    throw new Error('Moteur lip-sync non configuré : ajoute LIPSYNC_SERVICE_URL sur Render.');
  }
