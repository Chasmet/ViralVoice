...
function reconcileSpeakerProfiles(profiles, speakers) {
  ...
  if (speakers.length === 2) {
    ...
    const firstProfile = output[first].profile;
    const secondProfile = output[second].profile;

    if (firstProfile === 'masculine' && secondProfile === 'neutral') {
      output[second] = { profile: 'feminine', confidence: 0.56, source: 'paired-inference' };
    } else if (firstProfile === 'feminine' && secondProfile === 'neutral') {
      output[second] = { profile: 'masculine', confidence: 0.56, source: 'paired-inference' };
    } else if (secondProfile === 'masculine' && firstProfile === 'neutral') {
      output[first] = { profile: 'feminine', confidence: 0.56, source: 'paired-inference' };
    } else if (secondProfile === 'feminine' && firstProfile === 'neutral') {
      output[first] = { profile: 'masculine', confidence: 0.56, source: 'paired-inference' };
    }
  }
