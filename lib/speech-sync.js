...
function calculateWordBudget(durationSeconds) {
  const duration = Math.max(0.25, Number(durationSeconds || 0));
  return Math.max(1, Math.min(42, Math.round(duration * 2.65)));
}
...
function normalizeDelivery(value) {
  const delivery = String(value || 'naturel et fidèle au dialogue')
    .replace(/[\r\n]+/g, ' ')
    .trim()
    .slice(0, 120);
  return delivery || 'naturel et fidèle au dialogue';
}

function normalizePace(value) {
  const pace = String(value || '').toLowerCase();
  if (pace.includes('lent')) return 'slow';
...
async function createJsonCompletion({ messages, jobId, label, maxCompletionTokens = 12000 }) {
  ...
  try {
    return await openai.chat.completions.create(primaryRequest);
  } catch (primaryError) {
...
async function translateTimedSegments(segments, targetLanguage, jobId) {
  const timingInput = segments.map((segment, id) => {
    const durationSeconds = Math.max(0.25, segment.end - segment.start);
    return {
      id,
      speaker: segment.speaker,
      originalText: segment.text,
      durationSeconds: Number(durationSeconds.toFixed(2)),
      maximumSpokenWords: calculateWordBudget(durationSeconds)
    };
  });
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Tu es adaptateur professionnel de doublage cinéma. Traduis vers la langue demandée ' +
            'et réécris chaque réplique pour qu’elle soit naturelle à l’oral et réellement prononçable ' +
            'dans sa durée. Préserve le sens, les noms propres, les faits, le registre, l’humour et ' +
            'l’intention. Utilise le contexte des autres répliques pour les pronoms et les références. ' +
            'Ne dépasse pas maximumSpokenWords, sauf nom propre indispensable. Évite le mot-à-mot, ' +
            'les formulations écrites et les préambules. Renvoie uniquement un JSON valide au format ' +
            '{"segments":[{"id":0,"text":"...","delivery":"émotion et intention brèves",' +
            '"pace":"slow|normal|fast"}]}. Conserve exactement tous les identifiants, leur nombre et leur ordre.'
        },
        {
          role: 'user',
          content: JSON.stringify({ targetLanguage, segments: timingInput })
        }
      ]
    });
    ...
    let output = segments.map((segment, index) => {
      ...
      return {
        ...segment,
        translatedText: String(item.text || segment.text).trim(),
        delivery: normalizeDelivery(item.delivery),
        pace: normalizePace(item.pace),
        wordBudget: calculateWordBudget(durationSeconds)
      };
    });

    output = await tightenOverlongSegments(output, targetLanguage, jobId);
    return output;
  } catch (error) {
    ...
    for (const segment of segments) {
      ...
      output.push({
        ...
        translatedText: await translateOne(
          segment.text,
          targetLanguage,
          calculateWordBudget(durationSeconds),
          jobId
        ) || segment.text,
...
async function tightenOverlongSegments(segments, targetLanguage, jobId) {
  const overlong = segments
    .map((segment, id) => ({
      ...
      actualWords: countWords(segment.translatedText)
    }))
      ...
      messages: [
        {
          ...
          content:
            ...
            'ni le ton. Respecte strictement maximumSpokenWords. Réponds uniquement avec ' +
            '{"segments":[{"id":0,"text":"..."}]} et conserve les identifiants fournis.'
        },
        {
          role: 'user',
    ...
    messages: [
      {
        ...
        content:
          ...
          'Réponds uniquement avec {"text":"..."}.'
      },
      {
        role: 'user',
        content: JSON.stringify({ targetLanguage, maximumSpokenWords, text: String(text).slice(0, 4000) })
      }
    ]
  });
  const parsed = JSON.parse(String(completion.choices?.[0]?.message?.content || ''));
...
function estimateSpeechSpeed(text, slotDuration, pace) {
  const words = Math.max(1, countWords(text));
  const naturalDuration = words / 2.65;
  const paceMultiplier = pace === 'fast' ? 1.08 : pace === 'slow' ? 0.92 : 1;
  return clamp((naturalDuration / Math.max(0.4, slotDuration)) * paceMultiplier, 0.82, 1.25);
}

function roleInstruction(role) {
  if (role === 'male') return 'timbre masculin naturel, adulte et crédible';
  if (role === 'female') return 'timbre féminin naturel, adulte et crédible';
  return 'timbre naturel, distinct et cohérent';
}

async function generatePremiumAudioSegment({
  text,
  voice,
  outputPath,
  slotDuration,
    ...
    messages: [
      {
        role: 'system',
        content:
          'Tu es un comédien professionnel de doublage. Prononce uniquement la réplique fournie, ' +
          'sans commentaire, sans guillemets lus et sans ajouter de mots. Garde une diction naturelle, ' +
          'une émotion crédible et une articulation propre.'
      },
      {
        role: 'user',
        content:
          `Réplique exacte : ${String(text || '').trim()}\n` +
          `Voix : ${roleInstruction(role)}.\n` +
          `Intention : ${normalizeDelivery(delivery)}.\n` +
          `Débit : ${normalizePace(pace)}.\n` +
          `Durée visée : environ ${slotDuration.toFixed(2)} secondes.`
      }
    ]
  });

  const encoded = completion.choices?.[0]?.message?.audio?.data;
  if (!encoded) throw new Error('Le modèle audio premium n’a renvoyé aucun son.');
  fs.writeFileSync(outputPath, Buffer.from(encoded, 'base64'));
}

async function generateSpeechFallback({
  text,
  voice,
  outputPath,
  slotDuration,
  ...
  const speed = estimateSpeechSpeed(text, slotDuration, pace);
  const speech = await openai.audio.speech.create({
    model: config.TTS_FALLBACK_MODEL,
    voice,
    input: String(text || '').trim().slice(0, 4000),
    instructions:
      `Utilise un ${roleInstruction(role)}. Intention : ${normalizeDelivery(delivery)}. ` +
      `Prononce uniquement la réplique, sans préambule. Vise environ ` +
      `${slotDuration.toFixed(2)} secondes avec une diction naturelle.`,
    speed,
    response_format: 'wav'
  });
...
async function generateVoiceSegment(options) {
  try {
    await generatePremiumAudioSegment(options);
    return { engine: config.AUDIO_DUB_MODEL, fallback: false };
  } catch (error) {
    console.warn('PREMIUM AUDIO FALLBACK', error.message || error);
    await generateSpeechFallback(options);
    return { engine: config.TTS_FALLBACK_MODEL, fallback: true };
  }
}
