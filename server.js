...
const MAX_DURATION_SECONDS = Number(process.env.MAX_DURATION_SECONDS || 120);
...
app.post('/api/client', async (req, res) => {
  try {
    ...
    res.json({ ok: true, client, wallet });
  } catch (error) {
...
app.post('/api/admin/add-tokens', async (req, res) => {
  try {
    ...
    const packName = cleanText(req.body.packName || 'Ajout manuel');
    ...
    const { client, wallet } = await addMinutesToWallet({
      email,
      minutes,
      packName,
      amountEur,
...
app.post('/api/dub-video', upload.single('media'), async (req, res) => {
  ...
  try {
    ...
    console.log(`[${jobId}] DURATION ${duration}s minutes=${minutesNeeded}`);

    if (duration > MAX_DURATION_SECONDS) {
      return res.status(400).json({
        error: `Fichier trop long. Limite actuelle : ${MAX_DURATION_SECONDS} secondes.`
      });
    }
    ...
    if (adminFreeMode) {
      ...
      const { client } = await consumeMinutes(effectiveClientEmail, minutesNeeded);
      chargedClient = client;
    ...
    await recordGeneration({
      ...
      prompt: adminFreeMode
        ...
        : `Doublage vers ${targetLanguage} - ${minutesNeeded} minute(s)`,
      voiceStyle: voice,
    ...
    if (chargedMinutes > 0 && chargedClient) {
      await refundMinutes(chargedClient.id, chargedMinutes).catch(refundError => {
...
async function addMinutesToWallet({ email, minutes, packName, amountEur, revolutPaymentId }) {
  ...
  const newBalance = Number(wallet.token_balance || 0) + minutes;
  const newPurchased = Number(wallet.total_tokens_purchased || 0) + minutes;
  ...
  return { client, wallet: updatedWallet };
}
...
async function consumeMinutes(email, minutesNeeded) {
  ...
  const { data: updatedWallet, error: updateError } = await supabase
    .from('token_wallets')
    .update({
      token_balance: currentBalance - minutesNeeded,
...
async function translateText(text, targetLanguage) {
  const safeText = String(text || '').slice(0, 9000);
