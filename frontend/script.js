...
saveBackendBtn.addEventListener('click', () => {
  ...
  if (!backend) return showBackendStatus('Colle une URL Render valide.', 'error');
...
mediaFile.addEventListener('change', () => {
  ...
  fileInfo.textContent = `${isVideo ? 'Vidéo' : 'Audio'} : ${file.name} • ${sizeMb} MB`;
...
async function testBackendConnection() {
  ...
  if (!backend) return showBackendStatus('Aucun service configuré.', 'error');
  ...
  try {
    ...
    if (!response.ok || !data.ok) throw new Error('Service invalide');
...
async function createDub() {
  ...
  if (!backend) return showUserStatus('Service indisponible pour le moment.', 'error');
  if (!file) return showUserStatus('Choisis une vidéo ou un audio.', 'error');
  if (file.size > MAX_FILE_SIZE) return showUserStatus('Fichier trop lourd pour la V1. Coupe une vidéo plus courte.', 'error');
  ...
  try {
    ...
    const response = await fetch(`${backend}/api/dub-video`, { method: 'POST', body: formData });
...
function scheduleStatusMessages() {
  setTimeout(() => { if (!statusCard.classList.contains('hidden')) statusText.textContent = 'Transcription de la voix originale...'; }, 2500);
  setTimeout(() => { if (!statusCard.classList.contains('hidden')) statusText.textContent = 'Traduction du texte...'; }, 7000);
  setTimeout(() => { if (!statusCard.classList.contains('hidden')) statusText.textContent = 'Création de la voix IA...'; }, 12000);
  setTimeout(() => { if (!statusCard.classList.contains('hidden')) statusText.textContent = 'Préparation du fichier final...'; }, 18000);
}
...
async function readJsonResponse(response) {
  ...
  catch (error) { return { error: text || 'Réponse serveur illisible' }; }
}
