...
/*
  Version Render gratuit sécurisée :
  - 25 MB maximum
  - 60 secondes maximum
  - FFmpeg limité
  - 1 traitement à la fois
*/
...
app.get('/api/health', (req, res) => {
  res.json({
    ok: true,
    app: 'ViralVoice API',
    version: '1.0.1-memory-safe',
    openaiKey: Boolean(process.env.OPENAI_API_KEY),
    openAiDiarization: false,
    ffmpeg: true,
...
app.post('/api/dub-video', upload.single('media'), async (req, res) => {
  ...
  try {
    ...
    if (duration > MAX_DURATION_SECONDS) {
      return res.status(400).json({
        error: `Fichier trop long pour Render gratuit. Limite actuelle : ${MAX_DURATION_SECONDS} secondes.`
      });
    }
