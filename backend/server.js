...
/*
  Crée ou récupère un client avec son portefeuille.
  Body:
  {
...
/*
  Voir le solde de jetons d'un client.
  Exemple:
  /api/wallet?email=client@email.com
*/
...
/*
  Ajout manuel de jetons après paiement Revolut Pro.

  Sécurité:
  - Ajoute dans Render une variable ADMIN_SECRET
  - Puis appelle cette route avec le header:
    x-admin-secret: ton_secret
...
app.post('/api/admin/add-tokens', async (req, res) => {
  try {
    ...
    if (!Number.isInteger(tokens) || tokens <= 0) {
      return res.status(400).json({
        error: 'Nombre de jetons invalide.'
      });
    }
    ...
    res.json({
      ok: true,
      message: `${tokens} jetons ajoutés au client ${email}.`,
      client,
      wallet
    });
  } catch (error) {
    res.status(500).json({
      error: error.message || 'Erreur ajout jetons.'
    });
  }
});
...
app.post('/api/dub-video', upload.single('media'), async (req, res) => {
  ...
  try {
    ...
    if (!clientEmail) {
      return res.status(400).json({
        error: 'Email client obligatoire pour utiliser les jetons.'
      });
    }
    ...
    const { client } = await consumeOneToken(clientEmail);
...
function requireAdmin(req) {
  if (!ADMIN_SECRET) {
    throw new Error('ADMIN_SECRET manquant dans Render. Ajoute une variable ADMIN_SECRET pour protéger l’ajout manuel de jetons.');
  }
...
async function recordGeneration({ clientId, prompt, voiceStyle, resultUrl, status }) {
  const { error } = await supabase
    ...
    .insert({
      ...
      tokens_used: status === 'completed' ? 1 : 0,
